...
fun LoginPage() {
    ...
    Box(Modifier.fillMaxSize().backgroundColor(Color("#f5f5f5")), contentAlignment = Alignment.Center) {
        Column(
            ...
            H1(attrs = Modifier.margin(bottom = 24.px).fontSize(24.px).toAttrs()) { Text("Sign in") }

            FormInput(label = "Email", value = email, type = "email", placeholder = "you@example.com") { email = it }
            FormInput(
                label = "Password",
                value = password,
                type = "password",
