...
suspend fun updateNote(ctx: ApiContext) {
    ...
    transaction {
        ...
        val updated = Notes.update({ Notes.id eq request.id }) {
            it[title] = request.title
            it[content] = request.content
            it[color] = request.color
            it[updatedAt] = System.currentTimeMillis()
        }
        if (updated == 0) ctx.respondError(404, "Note not found")
        else ctx.respondJson(200, apiJson.encodeToString(SuccessResponse("Note updated")))
    }
}
